...
import {
    ...
    AuthorizeCodeSignInForBuyerInput
} from "GraphType";
...
export const AuthorizeCodeSignInForBuyerFunc = async (
    { args },
    stack: any[]
): Promise<AuthorizeCodeSignInForBuyerResponse> => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const {
            param: { authorizeCode, redirectUri }
        }: { param: AuthorizeCodeSignInForBuyerInput } = args;
        stack.push("뀨?");
...
const resolvers: Resolvers = {
    Query: {
        AuthorizeCodeSignInForBuyer: defaultResolver(
            AuthorizeCodeSignInForBuyerFunc
        )
    }
};
