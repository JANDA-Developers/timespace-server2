...
import { ItemModel, ItemCls } from "./Item";
...
export const findDuplicatedItems = async (
    item: DocumentType<ItemCls>,
    includeStatuses: ItemStatus[] = ["PENDING"]
): Promise<DocumentType<ItemCls>[]> => {
    const { dateTimeRange } = item;
    const from = dateTimeRange.from as Date;
    const to = dateTimeRange.to as Date;
    const query = {
        _id: {
            $ne: item._id
        },
    ...
    const duplicatedItems = ItemModel.find(query).exec();
